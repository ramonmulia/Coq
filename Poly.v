...
Definition negf {X:Type} (f: X -> bool) (x : X) : bool := negb (f x).

Definition partition {X : Type} (f : X -> bool) (l : list X) 
                     : list X * list X :=
(filter f l, filter (fun l' => negf f l') l).
...
Lemma snoc_simpl : forall (X:Type) (s:X) (l:list X) ,
  snoc l s = l ++ [s].
Proof.
...
Lemma map_on_fst {X Y:Type} (h:X) (t: list X) (f:X->Y) : map f (h :: t) = (f h) :: map f t.
Proof.
  intros.
  simpl.
  reflexivity.
...
Lemma map_on_lst {X Y:Type} (h:X) (t: list X) (f:X->Y) : map f (t ++ [h]) = (map f t) ++ [f h].
Proof.
  intros. 
  induction t.
  reflexivity.
...
Lemma distr_rev2 {X:Type} (x:X) (l:list X) : rev (x :: l) = (rev l) ++ [x].
Proof.
  intros. simpl. rewrite snoc_simpl. reflexivity.
Qed. 
...
Proof.
  ...
  reflexivity.
  rewrite distr_rev2.
  rewrite map_on_lst. rewrite IHl. rewrite map_on_fst.
  simpl.
  rewrite snoc_simpl.
  reflexivity.
Qed.
...
Definition fmostlytrue := override (override (constfun true) 1 false) 3 false.

Check (fold plus).
Eval simpl in (override (override (constfun true) 1 false) 3 false 0).

Example override_example1 : override (override (constfun true) 1 false) 3 false 0 = true.
Proof. reflexivity. Qed.
...
Proof.
  intros X x1 k1 k2 f.
  intros H. unfold override.
  remember (beq_nat k1 k2) as r. 
  destruct r.
  Case "r = true".
    rewrite <- H. apply beq_nat_eq in Heqr. 
...
Proof.
   ...
   intros contra.
   inversion contra. 
   intros lf. intros H. inversion H. remember (test x0) as r.
    destruct r.
    SCase "r = true".
      inversion H1. rewrite <- H2. symmetry. assumption.
    SCase "r = false".
      apply IHl with (lf := lf). assumption.
Qed.
